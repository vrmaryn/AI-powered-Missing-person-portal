...
if st.session_state.get("authentication_status"):
    ...
    st.markdown(
        """
        <div class="app-hero">
            <h3 style="margin: 0;">Missing Person Intelligence Dashboard</h3>
            <p style="margin: 0.25rem 0 0 0;">Track active cases, monitor outcomes, and coordinate response faster.</p>
        </div>
        """,
        unsafe_allow_html=True,
    )
    st.write(
        f'<p style="color:grey; text-align:left; font-size:40px; margin-bottom: 0;">{user_info["name"]}</p>',
        unsafe_allow_html=True,
    )

    st.write(
        f'<p class="app-subtle">{user_info["area"]}, {user_info["city"]}</p>',
        unsafe_allow_html=True,
    )

    st.write(
        f'<p class="app-subtle">Role: {user_info["role"]}</p>',
        unsafe_allow_html=True,
    )
    ...
    st.caption("Use the sidebar to register new cases, review existing cases, and run match checks.")
